...
export const AnimatedWave: React.FC<AnimatedWaveProps> = ({
    ...
    const animate = () => {
      ...
      for (let x = 0; x < gridSize; x++) {
        for (let y = 0; y < gridSize; y++) {
          ...
          const n2 =
            Math.sin(posX * 0.001 + time * 2) *
            Math.cos(posZ * 0.001 + time) *
            (amplitude * 0.4);
          let height = n1 + n2;
          ...
          if (mouseInteraction) {
            ...
            if (dist < 280) {
              ...
              height +=
                Math.sin(dist * 0.04 - time * 6) * (amplitude * 1.5) * rippleFactor;
            }
          }
  ...
  return (
    ...
    >
      {webGLFailed && (
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-background/90 p-6 text-center text-foreground">
          <p className="text-sm font-semibold">🚫 WebGL Support Required</p>
          <p className="mt-1 max-w-xs text-xs text-muted-foreground">
            Unable to render the 3D animated wave. Please enable hardware
