  ...
  return (
    <>
      ...
      {(open === "products" || open === "services") && (
        ...
        >
          <div className="bg-black/90 backdrop-blur-2xl border-b border-white/[0.07] shadow-2xl shadow-black/60">
                ...
                >
                  <div className="w-full py-4 grid grid-cols-3 gap-6">
                    {PRODUCTS.map((p) => (
                      <div
                        key={p.id}
                        className={`group flex flex-col gap-5 p-7 rounded-3xl border ${p.border} ${p.color} transition-all duration-200 hover:-translate-y-0.5 hover:shadow-xl hover:shadow-black/30`}
                      >
                        <div className="flex items-start gap-4">
                          <div className={`flex-shrink-0 w-12 h-12 flex items-center justify-center rounded-xl ${p.iconBg} ${p.accent} [&_svg]:w-6 [&_svg]:h-6`}>{p.icon}</div>
                          <div>
                            <div className="text-white font-bold text-lg leading-tight">{p.name}</div>
                            <div className={`text-sm mt-1 ${p.accent}`}>{p.tagline}</div>
                          </div>
                        </div>
                        ...
                        <ul className="flex flex-col gap-2">
                          {p.features.map((f) => (
                            <li key={f} className="flex items-center gap-2 text-sm text-white">
                              <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${p.accent}`} style={{ background: "currentColor" }} />
                              {f}
                            </li>
                          ))}
                        </ul>
                  ...
                  <div className="w-full py-4 grid grid-cols-3 gap-6">
                    {SERVICES.map((s) => (
                      <Link key={s.id} to="#services"
                            className="group flex items-start gap-4 p-7 rounded-3xl border border-white/[0.08] bg-[#0a0a0d] hover:border-white/[0.18] hover:bg-[#111114] transition-all duration-200 hover:-translate-y-0.5 hover:shadow-xl hover:shadow-black/30">
                        <div className={`flex-shrink-0 w-12 h-12 flex items-center justify-center rounded-xl ${s.iconBg} ${s.accent} mt-0.5 [&_svg]:w-6 [&_svg]:h-6`}>{s.icon}</div>
                        <div>
                          <p className={`text-base font-bold text-white group-hover:${s.accent} transition-colors leading-snug mb-2`}>{s.name}</p>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 md:hidden flex flex-col" style={{ top: "68px" }}>
          <div className="flex-1 bg-black/90 backdrop-blur-2xl overflow-y-auto">
                ...
                <div className="mt-1 mb-2 flex flex-col gap-1.5 pl-2">
                  {SERVICES.map((s) => (
                    <Link key={s.id} to="#services" onClick={closeMobile}
                          className="flex items-center gap-3 p-3 rounded-xl border border-white/[0.08] bg-[#0a0a0d] hover:bg-[#111114] transition-colors">
